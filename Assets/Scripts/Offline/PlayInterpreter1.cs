...
public class PlayInterpreter1 : MonoBehaviour
{
    PlayTerminalManager1 terminalManager;
    ...
    private void Start()
    {
        terminalManager = GetComponent<PlayTerminalManager1>();
    }
    ...
    public List<string> Interpret(string userInput)
    {
        ...
        if (args[0] == "help")
        {
            ...
            ListEntry("return", "Leaves the game and returns to the main terminal screen.");
        ...
        if(args[0] == "move" && args[1] != null && args[2] != null)
        {
            ...
            {
                player.GetComponent<PlayerManager1>().directLR = args[1];
                player.GetComponent<PlayerManager1>().unitsLR = (float)units;
            }
            else
            {
                player.GetComponent<PlayerManager1>().directUD = args[1];
                player.GetComponent<PlayerManager1>().unitsUD = (float)units;
            }
            return response;
        }
        ...
        {
            player.GetComponent<PlayerManager1>().dashDir = args[1];
            dashTimer = 30f;
            return response;
        }
